...
- 그래서 정적 데이터를 처리하는 `Web Server(NGINX)`를 배포 환경에서 사용하였고 클라이언트가 `HTML, CSS와 같은 정적 데이터`를 요청하면 `NGINX 서버`에서 클라이언트에게 데이터를 제공하고, `동적 데이터(DB 데이터)`를 요청할 경우 클라이언트의 요청을 `WAS`에 전달하고 WAS가 처리한 데이터를 클라이언트에게 제공하도록 하였습니다.
</details>
